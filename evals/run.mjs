 ...
 *     uses `--bare`, which also skips hooks, auto-memory and CLAUDE.md
 *     auto-discovery; on an OAuth session `--bare` cannot authenticate, so it
 *     falls back to `--setting-sources project`. That fallback is weaker — it
 *     skips the user *settings* but leaves `~/.claude/CLAUDE.md` and hooks
 *     in place — so the mode is printed with the results and a run under it is
 *     not evidence about routing on its own.
 ...
 *   - `--tools` restricts the toolset; `--allowedTools` would only grant
 *     permission and leave Grep in place, and Grep in content mode reads a
 *     skill body without naming a file, which makes every `readsNone` vacuous.
...
function askAgent(dir, prompt, readOnly = false) {
  ...
  for (const line of raw.split('\n')) {
    ...
    if (event.type === 'result') {
      said += event.result || '';
      if (event.is_error) failed = failed || (event.result || 'error').slice(0, 160);
    }
  }
  return { opened, said, failed };
}
...
function snapshot(dir, files) {
  return Object.fromEntries(
    files.map((f) => {
      ...
      // null on both sides would compare equal and delete the assertion.
      if (!existsSync(full)) throw new Error(`unchanged path is not in the fixture: ${f}`);
      return [f, readFileSync(full, 'utf8')];
    }),
  );
}
...
if (worst === 1) {
  ...
  console.log(`\nisolation: ${ISOLATION.join(' ')}`);
} else {
  console.log(`\nisolation: ${ISOLATION.join(' ')}   work tree kept: ${WORK}`);
}
