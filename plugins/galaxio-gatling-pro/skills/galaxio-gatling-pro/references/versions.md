...
- **Gatling itself** is `gatling-charts-highcharts`, `gatling-test-framework` and `gatling-app`,
...
- **Java 8 on 3.9.x is a Scala-only floor.** `gatling-core` and the Galaxio libraries' Scala
  classes are class-file major 52 there, but the `org.galaxio.gatling.javaapi` facade every Java
  and Kotlin project goes through is major 61 in the same jars — measured in Picatinny `0.14.1`
