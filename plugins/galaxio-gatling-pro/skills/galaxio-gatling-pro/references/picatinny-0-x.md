...
adds `RandomProvider` and `RandomDataGeneratorsWrapper`.
...
A 3.11 project on a `1.x` pin belongs there, not here. For a project without the dependency,
...
the version comes from the project's line: `0.14.x` on 3.9.x, `0.15.0` on 3.10.x, `0.16.0` and up
on 3.11.x. See [versions.md](versions.md).
...
This line also carries `profile`, `templates` and an `influxdb` integration that was already
deprecated at `0.18.2`.
...
[picatinny-1-x.md](picatinny-1-x.md). Code that reaches for `getOptStringParam` or `.orElse(…)`
does not compile here — use the two-argument default form instead.
...
Moving `0.x` to `1.x` is a real API change. From a
