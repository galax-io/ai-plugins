...
The profiles record where a library _crosses_ a line, and on a closed line both ends of the range.
None of them records how far an open line has got — that is a snapshot, so it is a command here
rather than a number there. Quote the output with the date you ran it.
...
`gatling-app`, and `io/gatling/highcharts/gatling-charts-highcharts`. Two build plugins carry a
suffix the artifact name does not show: `io/gatling/gatling-maven-plugin` is plain, but the sbt
plugin is cross-built and lives at `io/gatling/gatling-sbt_2.12_1.0`. `io.gatling.gradle` is a
plugin marker rather than a library, so it sits on the Gradle plugin portal at
...
Run it whenever a pin looks out of sequence: version order alone does not tell you the line, and
[versions.md](versions.md) has two published counter-examples.
...
Shape rules, the same on every line — the profiles carry the numbers and nothing else:

- The numbers go into the build file for your language and build tool; the dispatch table in
  `SKILL.md` names which one.
...
- Nothing outside a profile's table is a hard pin — match what the repository declares.
