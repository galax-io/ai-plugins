...
When the task will not register, `gatlingClasses` still compiles and the simulation can be run off the `gatlingRuntimeClasspath` with `io.gatling.app.Gatling -s <fqcn>`.
